...
class ExplainQueryAction(private val project: Project, private val queryTree: Tree) :
    BaseQueryAction("Explain Query", "", Icons.ICON_PROFILE) {

    override fun actionPerformed(e: AnActionEvent) {
        val query = getSelectedQuery(project, queryTree)
        if (query == null) { log.error("Nothing selected"); return }
        AqlDataService.with(project).explainQuery(query.query ?: "", query.getParameters())
    }
}
